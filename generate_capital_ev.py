...
EV_TEMPLATE = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>EV Drive: {from_name} to {to_name} | {hours}h {mins}min, {charges} Charging Stops</title>
    <meta name="description" content="Electric vehicle route from {from_name} to {to_name}. {hours}h {mins}min drive time, {miles} miles, {charges} charging stops needed.">
    <link rel="canonical" href="https://howlongdrive.com/ev/{slug}/">
    <link rel="icon" type="image/x-icon" href="/favicon.ico">
    <meta name="theme-color" content="#10B981">
    <meta property="og:title" content="EV Route: {from_name} to {to_name}">
    <meta property="og:description" content="EV drive time: {hours}h {mins}min, {charges} charging stops">
...
<body>
    ...
    <main class="container">
        <div class="breadcrumb"><a href="/">Home</a> → <a href="/ev/">EV Routes</a> → {from_short} to {to_short}</div>
        <div class="hero">
            <h1>⚡ EV: {from_name} to {to_name}</h1>
        ...
        <div class="grid">
            <div class="card">
                ...
                <div class="row"><span>Charge Time</span><span>~{charge_time}min</span></div>
            </div>
            <div class="card">
                <h3>💰 Cost Comparison</h3>
                <div class="row"><span>EV Charging</span><span>${charge_cost}</span></div>
                <div class="row"><span>Gas Car</span><span>${gas_cost}</span></div>
                <div class="row"><span>Savings</span><span style="color:#10B981">${savings}</span></div>
            </div>
        </div>
        <div id="map"></div>
    ...
    <script>
        ...
        L.polyline([[{from_lat}, {from_lon}], [{to_lat}, {to_lon}]], {{color: '#10B981', weight: 3}}).addTo(map);
    </script>
</body>
</html>'''
...
for r in routes:
    ...
    if os.path.exists(f"ev/{slug}/index.html"):
        continue
    
    # EV calculations (300 mile range, 30min charges)
    ...
    charge_cost = int(r['miles'] * 0.04)  # ~$0.04/mile for EV
    gas_cost = int(r['miles'] / 30 * 3.50)
    savings = gas_cost - charge_cost
...
print(f"✅ Generated {count} new EV capital route pages")
